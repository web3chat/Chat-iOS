...
public class Ping : NSObject {
    ...
    static func icmp4HeaderOffsetInPacket(_ packet : Data) -> UInt{
        ...
        if packet.count >= MemoryLayout<IPHeader>.size + MemoryLayout<ICMPHeader>.size{
            ipPtr = packet.pingbytes.bindMemory(to: IPHeader.self, capacity:packet.count).pointee
    ...
    static func icmp4InPacket(_ packet:Data) -> ICMPHeader?{
        ...
        if icmpHeaderOffset != NSNotFound {
            let uint8Bytes = packet.pingbytes.bindMemory(to: UInt8.self, capacity: packet.count) + Int(icmpHeaderOffset)
    ...
    static func sourceAddressInPacket(_ packet:Data) ->String?{
        ...
        if packet.count >= MemoryLayout<IPHeader>.size{
            ipPtr = packet.pingbytes.bindMemory(to: IPHeader.self, capacity: packet.count)
    ...
    var hostAddressFamily : sa_family_t {
        get{
            ...
            if let hostAddress = self.hostAddress, hostAddress.count >= MemoryLayout<sockaddr>.size{
                result = hostAddress.pingbytes.bindMemory(to: sockaddr.self, capacity: hostAddress.count).pointee.sa_family
            }
            return result
        }
    }
    ...
    func isValidPing4ResponsePacket(_ packet : Data) -> Bool{
        ...
        if icmpHeaderOffset != NSNotFound{
            let uInt8poiner = packet.pingbytes.bindMemory(to: UInt8.self, capacity: packet.count) + Int(icmpHeaderOffset)
    ...
    func isValidPing6ResponsePacket(_ packet:Data)->Bool{
        ...
        if packet.count >= MemoryLayout<ICMPHeader>.size {
            icmpPtr = packet.pingbytes.bindMemory(to: ICMPHeader.self, capacity: packet.count)
            if icmpPtr.pointee.type == kICMPv4Type.EchoReply.rawValue,icmpPtr.pointee.code == 0{
                if CFSwapInt16(icmpPtr.pointee.identifier) == self.identifier{
                    if CFSwapInt16(icmpPtr.pointee.sequenceNumber) < self.nextSequenceNumber{
                        result = true
    ...
    func listenPacket(){
        ...
        if bytesRead > 0 {
            ...
            let point = UnsafeMutablePointer<CChar>(&hoststr)
           
            let host = String(utf8String: point)
            ...
            if(host == hostAddressString) { // only make sense where received packet comes from expected source
                ...
                if sin.sin_family == AF_INET{
                    ...
                    headerPointer = packet.pingbytes.bindMemory(to: ICMPHeader.self, capacity: packet.count).pointee
    ...
    func pingPacketWithType(_ type: UInt8, _ payload:Data ,_ requiresChecksum:Bool)  -> NSData{
        ...
        memcpy(&icmpPtr[1], payload.pingbytes, payload.count);
        if requiresChecksum {
    ...
    func sendPacket(){
        if self.isPinging {
            ...
            if self.socketNum == 0{
                ...
                bytesSent = sendto(self.socketNum, packet.bytes, packet.length, 0, hostAddress.pingbytes.bindMemory(to: sockaddr.self, capacity: hostAddress.count), socklen_t(hostAddress.count))
                err = 0
                if bytesSent < 0 {
                    err = Int(errno)
                }
...
extension Data{
    var pingbytes : UnsafeRawPointer{
        get{
