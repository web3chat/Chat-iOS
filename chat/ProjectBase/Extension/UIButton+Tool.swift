...
extension UIButton{
    ...
    func setBackgroundColor(color:UIColor,state:UIControl.State){
        self.setBackgroundImage(UIImage.imageWithColor(with: color, size: CGSize(width: 1, height: 1)), for: state)
    }
    
    class func getNormalBtn(with title: String , backgroundColor: UIColor = Color_Theme) -> UIButton {
        let btn = UIButton(type: .custom)
        btn.makeOriginalShdowShow()
        btn.layer.cornerRadius = 20
        
        btn.layer.backgroundColor = backgroundColor.cgColor
        btn.setAttributedTitle(NSAttributedString(string: title, attributes: [.foregroundColor: backgroundColor == Color_Theme ? UIColor.white : Color_8A97A5,.font:UIFont.regularFont(16)]), for: .normal)
        return btn
    }
}
