...
- (void)textValueDidChanged:(UITextField *)textField {
    ...
    if (textField.text.length < 1) {
        self.unitLab.textColor = CMColorFromRGB(0xC8CEE5);
    }
    else {
        self.unitLab.textColor = CMColorFromRGB(0x8E92A3);
    }
...
- (void)changeTextFieldWidth:(UITextField *)textField {
    ...
    if (textFieldWidth <= self.frame.size.width) {
        ...
        CGFloat labWidth = [self stringWidthWithText:[PWUtils getUnitStr] font:font];
...
- (UIFont *)getFontWithText:(NSString *)text {
    ...
    NSString *content = [NSString stringWithFormat:@"%@%@",text,[PWUtils getUnitStr]];
...
- (UITextField *)textField {
    if (!_textField) {
        ...
        _textField.textColor = CMColorFromRGB(0x8E92A3);
        _textField.placeholder = @"0.00";
        [_textField setValue:CMColorFromRGB(0xC8CEE5) forKeyPath:@"placeholderLabel.textColor"];
